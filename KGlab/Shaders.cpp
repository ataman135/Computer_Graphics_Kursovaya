...
int loadShader(const char* filename, char** shaderSrc, int* programLength)
{
    ...
    *shaderSrc = (char*)malloc((*programLength + 1) * sizeof(char));
    DWORD nBytesRead = 0;
    ReadFile(file, *shaderSrc, *programLength, &nBytesRead, 0);
    (*shaderSrc)[*programLength] = '\0';
    CloseHandle(file);
    return 1;
}
...
void Shader::LoadShaderFromFile()
{
    ...
    if (program != 0) glDeleteObjectARB(program);
    if (vertex != 0) glDeleteObjectARB(vertex);
    if (fragment != 0) glDeleteObjectARB(fragment);

    program = glCreateProgramObjectARB();
    vertex = glCreateShaderObjectARB(GL_VERTEX_SHADER_ARB);
    fragment = glCreateShaderObjectARB(GL_FRAGMENT_SHADER_ARB);

    char* shader1Text = nullptr;
    char* shader2Text = nullptr;
    int shader1TextSize = 0;
    int shader2TextSize = 0;
...
void Shader::Compile()
{
    glCompileShaderARB(fragment);
    {
        ...
        if (length > 0) {
            ...
            if (!compiled) MessageBoxA(0, log, "Fragment Shader error!", 0);
            delete[] log;
        }
    }
    ...
    glCompileShaderARB(vertex);
    {
        ...
        if (length > 0) {
            ...
            if (!compiled) MessageBoxA(0, log, "Vertex Shader error!", 0);
            delete[] log;
        }
    }
    ...
    glLinkProgramARB(program);
    {
        ...
        if (length > 0) {
            ...
            if (!result) MessageBoxA(0, log, "Shader error!", 0);
            delete[] log;
        }
    }
}
